...
class Render:
    def __init__(self, entitiesSizeList, entitiesColorList, numAgents, getPosFromState, forestsID = None):
        ...
        for timeStep in range(len(trajectory)):
            ...
            if self.render_geoms is None:
                ...
                for entityID in range(self.numEntities):
                    entitySize = self.entitiesSizeList[entityID]
                    entityColor = self.entitiesColorList[entityID]
                    geom = make_circle(entitySize)
                    xform = Transform()
                    if entityID < self.numAgents: #entity is an agent
                        geom.set_color(*entityColor, alpha=0.5)
                    else:
                        geom.set_color(*entityColor)
                        if self.forestsID is not None:
                            if entityID in self.forestsID:
                                geom.set_color(*entityColor, alpha=0.5)
